...
function NavBar() {
  ...
  const handelScroll = () => {
   ...
   /* const servicesElement = document.querySelector('#services');*/
    ...
    if (scrollPosition <= homeElement.offsetTop && scrollPosition < aboutElement.offsetTop) {
      ...
      setActive("#about");
    } else if (scrollPosition <= experienceElement.offsetTop && scrollPosition < contactElement.offsetTop) {
  ...
  return (
  <nav>
    <a href="#home" onClick={() => setActive("#home")} className={active === '#home' ? 'active' : ''}><TiHomeOutline/></a>
